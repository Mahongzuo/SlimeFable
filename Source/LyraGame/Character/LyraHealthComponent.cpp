...
void ULyraHealthComponent::HandleOutOfHealth(AActor* DamageInstigator, AActor* DamageCauser, const FGameplayEffectSpec* DamageEffectSpec, float DamageMagnitude, float OldValue, float NewValue)
{
	...
	if (AbilitySystemComponent && DamageEffectSpec)
	{
		...
		// Send a standardized verb message that other systems can observe
		{
			...
			// SlimeFable: the ASC lives on the pawn (ALyraCharacterWithAbilities), so the effect instigator is the
			// pawn, not the PlayerState Lyra's HUD expects (W_Reticle_* casts Instigator to LyraPlayerState).
			// Resolve to the PlayerState when there is one; AI shooters have none and keep the pawn.
			Message.Instigator = DamageInstigator;
			if (APlayerState* InstigatorPS = ULyraVerbMessageHelpers::GetPlayerStateFromObject(DamageInstigator))
			{
				Message.Instigator = InstigatorPS;
			}
