...
void ALyraPlayerState::PostInitializeComponents()
{
	...
	if (World && World->IsGameWorld() && World->GetNetMode() != NM_Client)
	{
		...
		if (!GameState)
		{
			UE_LOG(LogLyra, Warning, TEXT("ALyraPlayerState::PostInitializeComponents: GameState is not ready yet."));
			return;
		}
		...
		{
			// SlimeFable play maps can use Lyra PlayerState without a Lyra Experience.
			UE_LOG(LogLyra, Warning, TEXT("ALyraPlayerState::PostInitializeComponents: no ExperienceManagerComponent on %s. Skipping Experience bind."), *GetNameSafe(GameState));
			return;
		}
