...
ULyraAssetManager& ULyraAssetManager::Get()
{
	...
	UE_LOG(LogLyra, Error, TEXT("AssetManager is not LyraAssetManager (class=%s). Set AssetManagerClassName=/Script/LyraGame.LyraAssetManager in DefaultEngine.ini. Using a transient fallback so the editor can keep running."),
		GEngine->AssetManager ? *GEngine->AssetManager->GetClass()->GetName() : TEXT("null"));

	static ULyraAssetManager* Fallback = nullptr;
	if (Fallback == nullptr)
	{
		Fallback = NewObject<ULyraAssetManager>(GetTransientPackage(), NAME_None, RF_Transient);
		if (Fallback)
		{
			Fallback->AddToRoot();
		}
...
void ULyraAssetManager::InitializeGameplayCueManager()
{
	...
	{
		UE_LOG(LogLyra, Warning, TEXT("LyraGameplayCueManager missing (set GlobalGameplayCueManagerClass). Skipping cue preload."));
	}
}
...
UPrimaryDataAsset* ULyraAssetManager::LoadGameDataOfClass(TSubclassOf<UPrimaryDataAsset> DataClass, const TSoftObjectPtr<UPrimaryDataAsset>& DataClassPath, FPrimaryAssetType PrimaryAssetType)
{
	...
	{
		UE_LOG(LogLyra, Error, TEXT("Failed to load GameData asset at %s. Type %s. Creating an empty in-memory asset so the editor can start."), *DataClassPath.ToString(), *PrimaryAssetType.ToString());
		Asset = NewObject<UPrimaryDataAsset>(GetTransientPackage(), DataClass, NAME_None, RF_Transient);
		if (Asset)
		{
			Asset->AddToRoot();
			GameDataMap.Add(DataClass, Asset);
