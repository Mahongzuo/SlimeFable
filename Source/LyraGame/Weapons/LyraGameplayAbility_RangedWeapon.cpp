...
void ULyraGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (bIsTargetDataValid && CommitAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo))
		{
			...
			// SlimeFable: non-GAS targets (slime, costume enemies) get plain damage instead.
			if (CurrentActorInfo->IsNetAuthority())
			{
				ApplyPlainDamageToNonAbilityTargets(LocalTargetDataHandle);
			}
		}
		else
		{
...
void ULyraGameplayAbility_RangedWeapon::ApplyPlainDamageToNonAbilityTargets(const FGameplayAbilityTargetDataHandle& TargetData)
{
	ULyraRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (!WeaponData || WeaponData->PlainHitDamage <= 0.f)
	{
		return;
	}

	APawn* InstigatorPawn = Cast<APawn>(GetAvatarActorFromActorInfo());
	AController* InstigatorController = GetControllerFromActorInfo();
	...
	for (int32 Index = 0; Index < TargetData.Num(); ++Index)
	{
		...
		float Damage = WeaponData->PlainHitDamage;
		Damage *= WeaponData->GetDistanceAttenuation(Distance);
		Damage *= WeaponData->GetPhysicalMaterialAttenuation(Hit->PhysMaterial.Get());
		if (Damage <= 0.f)
		{
			continue;
		}

		if (FLyraPlainWeaponDamage::OnHit.IsBound())
		{
			FLyraPlainWeaponDamage::OnHit.Execute(HitActor, Damage, *Hit, InstigatorPawn, WeaponData);
		}
		else
		{
			const FVector ShotDir = (Hit->ImpactPoint - Hit->TraceStart).GetSafeNormal();
			UGameplayStatics::ApplyPointDamage(HitActor, Damage, ShotDir, *Hit, InstigatorController, InstigatorPawn, nullptr);
		}
	}
}
