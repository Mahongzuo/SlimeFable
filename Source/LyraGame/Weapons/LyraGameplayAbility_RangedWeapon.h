...
protected:
	...
	/**
	 * SlimeFable: hits on actors without an AbilitySystemComponent never receive the damage GE.
	 * Route them through FLyraPlainWeaponDamage::OnHit (authority only).
	 */
	virtual void ApplyPlainDamageToNonAbilityTargets(const FGameplayAbilityTargetDataHandle& TargetData);
