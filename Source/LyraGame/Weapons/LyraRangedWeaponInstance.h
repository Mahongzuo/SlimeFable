...
/**
 * SlimeFable: authority-side hook fired by ULyraGameplayAbility_RangedWeapon for every hit on an actor that has
 * no AbilitySystemComponent (the slime, GASP costume enemies...). Lyra's damage GE can only reach ASC targets.
 * If unbound the ability falls back to UGameplayStatics::ApplyPointDamage.
 */
DECLARE_DELEGATE_FiveParams(FLyraPlainWeaponDamageDelegate, AActor* /*HitActor*/, float /*Damage*/, const FHitResult& /*Hit*/, APawn* /*InstigatorPawn*/, ULyraRangedWeaponInstance* /*Weapon*/);

struct LYRAGAME_API FLyraPlainWeaponDamage
{
	static FLyraPlainWeaponDamageDelegate OnHit;
};
...
public:
	...
	/**
	 * SlimeFable: damage per bullet applied (via FLyraPlainWeaponDamage::OnHit) to hit actors that have no
	 * AbilitySystemComponent. Scaled by DistanceDamageFalloff / MaterialDamageMultiplier like the GE path.
	 * 0 disables. ALyraShooterEnemy overwrites this at equip time from its own config.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Weapon Config|Plain Damage")
	float PlainHitDamage = 10.0f;
